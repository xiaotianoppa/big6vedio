    ...
    var jimScenarios = {
	  "bindScreenEvents" : function() {
	    ...
	    $(".scenarioShadow, #filterCloseButton").on('click', function (event) {
	      $(".scenarioShadow").fadeOut();
	      $(".filterDialog").fadeOut();
	    });
	  ...
	  "initializeScenarios" : function () {
		...
		if ($(".ui-scenario").length > 0) {
		  commentscontrol.css("opacity", 0);
		  highlight.css("opacity", 0);
		  separator.css("opacity", 0);
		  $('#highlight-select,#commentsSwitch').css('cursor', 'default');
		}
    	else {
    	  if (commentscontrol.hasClass("active")) {
    	    commentscontrol.css("opacity", '');
    		separator.not('#comments-separator1').css("opacity", '');
    		
    		$("#commentsSwitch").css('cursor', '');
    	  } 
    	  else {
    		commentscontrol.css("opacity", '');
    	    ...
    	    $('#highlight-select,#commentsSwitch').css('cursor', '');
    	  }
    	}
	  },
